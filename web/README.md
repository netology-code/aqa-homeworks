...
AppVeyor настраивается аналогично предыдущей лекции за исключением того, что тесты нужно вызывать так, чтобы selenide запускался в headless-режиме (читайте выше).
